...
class GenericKNXDevice extends Homey.Device {
  ...
  onInit() {
    ...
    const oldKNXInterface = this.homey.app.returnKNXInterface(this.macAddress);
    if (oldKNXInterface) {
      this.setKNXInterface(oldKNXInterface);
    } else {
      this.homey.app.once(`interface_found${this.macAddress}`, this.setKNXInterface.bind(this));
    }

    // Only to be executed when there is no interface AND no interfaces are found.
    this.homey.app.once('no_interfaces', () => {
      // Should try to obtain a KNX interface with the saved IP address
      const ip = this.getSetting('ipAddress');
      if (ip) {
        this.homey.app.discoverKNXInterfaceOnIP(ip)
          .catch(this.error);
      }
  ...
  onSettings(oldSettingsObj, newSettingsObj, changedKeysArr, callback) {
    ...
    if (this.knxInterface === undefined) {
      this.setKNXInterface(this.homey.app.returnKNXInterface(newSettingsObj.macAddress));
    }
    if ((this.knxInterface === undefined)
    && (oldSettingsObj.ipAddress !== newSettingsObj.ipAddress)) {
      this.homey.app.discoverKNXInterfaceOnIP(newSettingsObj.ipAddress)
        .catch(this.error);
    }
