...
class KNXGeneric extends Homey.Device {
    ...
    setKNXInterface(knxInterface) {
        ...
        if (this.hasCapability('measure_temperature')) {this.knxInterface.addKNXEventListener(this.settings.ga_temperature_measure, this.KNXEventHandler)}
        ...
        if (this.hasCapability('measure_temperature')) {this.knxInterface.addKNXEventListener(this.settings.ga_sensor, this.KNXEventHandler)}
    ...
    readSettingAddress(keys) {
        ...
        if (this.knxInterface.isConnected === true) {
            return Promise.all(settingKeys.map(settingKey => this.knxInterface.readKNXGroupAddress(this.getSetting(settingKey))))
                ...
                .catch(error => {
                    this.log('readsetting failed', error);
                });
        }
    }
    ...
    checkGroupAddressInUse(newSettingsObj) {
        //obtain all the drivers
                    ...
                    if (key.startsWith('ga_')) {
                        for (var newKey in newSettingsObj) {
                            if(allSettings[key] === newSettingsObj[newKey] && device !== this) {
                                this.log('Setting', newKey, 'is already in use by', device.getName());
                            }
                        }
                    }
                }
    ...
    onSettings(oldSettingsObj, newSettingsObj, changedKeysArr, callback) {
                ...
                for (var key in allSettings) {
                    if (key.startsWith('ga_')) {
                        for (var newKey in newSettingsObj) {
                            if(allSettings[key] === newSettingsObj[newKey] && device !== this) {
                                this.log('Setting', newKey, 'is already in use by', device.getName());
                                //return Promise.reject('Setting', newKey, 'is already in use by', device.getName());
                                callback('Settings could not be saved, groupaddress already in use!', null);
                            }
                        }
                    }
                }
        ...
        //return Promise.resolve(); //change to callbacks?
        callback(null, true);
    }
}
