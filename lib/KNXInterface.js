...
class KNXInterface extends EventEmitter {
	...
	createConnection() {
		...
		if (this.isConnected === true) return; // Skip re-initializing when already connected. // TODO change to promise?
		try {
	...
	_connectKNX() {
		...
		if (this.isConnected !== true) {
			...
			try { this.knxConnection.Connect();	}
			catch (error) { this.log('KNX lib connect error', error) }
		} else {
	...
	onKNXEventListener(event, source, destination, value) {
		...
		this.log(`Event' ${event} ${destination} ${value}`);
