...
class KNXInterface extends EventEmitter {
	constructor(knxInterface) {
		...
		this.knxConnection = new knx.Connection( {
			...
			handlers: {
				...
				error: (connstatus) => {
					this.log('Error from FSM:', connstatus, 'disconnecting tunnel');
		...
		// Enable this to automatically open the KNX connection.
		// Currently this is disabled because some IP interface only support a single connection, 
		// and we don't want to block it when not used by a device.
	...
	async readKNXGroupAddress(groupaddress) {
		if(groupaddress && groupaddress !== ""){
			...
			return Promise.reject('knx_no_connection'); //als new Error?
		} else {
