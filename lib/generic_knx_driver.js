    ...
    onPair(socket) {
        ...
        socket.on("manual_ip_address", (manualIPAddress, callback) => {
            ...
            Homey.app.discoverKNXInterfaceOnIP(this.manualInterfaceIP)
                ...
                .catch((error) => {
                    if (error === "interface_not_found") {
                        callback(Homey.__("settings.ip.interface_not_found"), null);
                    } else if (error === 'search_already_running') {
                        callback(Homey.__("settings.ip.search_already_running"), null);
                    } else {
                        callback((Homey.__("errors.generic_error")) + error);
                    }
                });
        });
        ...
        socket.on('return_selected_interface', (data, callback) => {
            if(this.selectedInterfaceMAC) {
                ...
                callback('no_matching_interface', null);
            }
