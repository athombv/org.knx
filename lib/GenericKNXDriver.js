...
class GenericKNXDriver extends Homey.Driver {
   ...
   */
  getInterfaceList() {
    return Object.values(this.homey.app.returnKNXInterfacesList()).map(knxInterface => {
      return {
  ...
  async onPair(session) {
    ...
    session.setHandler('search_interfaces', async () => {
      try {
        await this.homey.app.searchInterfaces();
        return this.getInterfaceList();
      } catch (error) {
    ...
    session.setHandler('manual_ip_address', async manualIPAddress => {
      try {
        await this.homey.app.discoverKNXInterfaceOnIP(manualIPAddress);
        return this.getInterfaceList();
      } catch (error) {
        throw new Error(this.homey.__(`errors.ip.${error.message}`));
      }
    ...
    session.setHandler('selected_interface', async mac => {
      if (mac && mac !== '') {
        ...
        this.homey.app.connectInterface(this.selectedInterfaceMAC);
      } else {
    ...
    session.setHandler('test_groupaddress', async data => {
      if (this.selectedInterfaceMAC) {
        ...
        return this.homey.app.testGroupAddress(this.selectedInterfaceMAC, data.address, data.time);
      }
      throw new Error();
    });
  }
