...
class KNXInterfaceManager extends EventEmitter {
	...
	// Thins method returns the complete list with KNXInterfaces
	getKNXInterfaceList() {
	...
	// Let the defined KNXINterface open the tunnel connection to the KNX network
	connectInterface(macAddress){
		this.KNXInterfaces[macAddress]._connectKNX();
	}

	// Search for KNX interfaces
	searchInterfaces() {
		this.findKNXInterfaces();
	}
