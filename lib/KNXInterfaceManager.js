...
class KNXInterfaceManager extends EventEmitter {
	...
	findKNXInterfaces() {
		...
		const knxport = 3671; //Default KNX IP port, rarely changed
		const knxmulticastaddr = '224.0.23.12'; // Default KNX multicast address
		const search_request = Buffer.from([0x06, 0x10, 0x2, 0x1, 0x00, 0x0e, 0x08, 0x01]) //Fixed KNX search bytes
		...
		udp_dgram
			...
			.on('message', (msg, rinfo) => {
				...
				this.KNXInterfaces[knxInterface.interfaceMac] = new KNXInterface(knxInterface); // Use the KNX topology address as the key value
		...
		udp_dgram.send([search_request, buffer_byte_ip, buffer_byte_port], knxport, knxmulticastaddr, function(err, bytes) {
			if (err) throw err;
			//console.log('UDP message sent to ' + knxmulticastaddr + ':' + knxport);
		});
	}
}
