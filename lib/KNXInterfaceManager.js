...
class KNXInterfaceManager extends EventEmitter {
	...
	findKNXInterfaces(ipAddress = '224.0.23.12') {
		...
		return new Promise((resolve, reject) => {
			...
			udp_dgram.send([search_request, buffer_byte_ip, buffer_byte_port], knxport, ipAddress, function(err, bytes) {
				if (err) throw err;
				//console.log('UDP message sent to ' + ipAddress + ':' + knxport);
			});
		});
	}
}
