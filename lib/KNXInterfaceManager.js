	...
	parseKNXResponse(inBuffer) {
		if (inBuffer[0] === 0x06 && inBuffer[1] === 0x10) {
			...
			if (inBuffer.readUInt16BE(2) === 0x202) {
				...
				console.log('Found', interfaceName, '@ IP:' + interfaceIp, 'with KNX address: ', knxAddress);
			...
			// Description response
				...
				console.log('Found', interfaceName, ' with KNX address:', knxAddress);
			...
			// Disconnect response
				...
				console.log('KNX connection disconnected');
				return {type: 0x0209};
			}
		} else {
			console.log('Response received, but not a valid KNX connection response');
			return null;
		}
	}
