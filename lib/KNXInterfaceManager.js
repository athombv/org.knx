...
const FIND_DEVICES_INTERVAL = 1 * (60 * 1000); // n minute search loop
...
class KNXInterfaceManager extends EventEmitter {
	...
	constructor() {
		...
		Homey.ManagerCloud.getLocalAddress()
			.then((address) => {
				if (address.split(':', 1).toString() === ip.address()) { this.homeyIP = ip.toBuffer(ip.address()) }
			...
			return new Promise((resolve, reject) => {
				udpSocket
					...
					.on('listening', () => {
						...
						udpSocket.setBroadcast(true); // Set the broadcast flag since the search request is using multicast.
						udpSocket.send(knxSearchRequest, knxIPPort, knxMultiCastAddress, 
							function(err, bytes) {
								if (err) {
									console.log(err);
									return reject ('dgram send error', err);
								}
							}
						);
					})
				...
				setTimeout(() => {
					...
					this.log('Current interfaces:', Object.values(this.KNXInterfaces).map((tempInterface) => { return tempInterface.name }));
					if (this.KNXInterfaces.length < 1) { this.emit('no_interfaces') }
