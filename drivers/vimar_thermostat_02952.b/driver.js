...
class VimarThermostat02952BDriver extends KNXGenericDriver {
  ...
  onInit() {
    ...
    this.homey.flow.getActionCard('set-window-switch').registerRunListener(async (args, state) => {
      ...
      await args.device.knxInterface.writeKNXGroupAddress(args.device.settings.ga_window_switch, args.open, 'DPT1')
        .catch((knxerror) => {
          this.log(knxerror);
    ...
    this.homey.flow.getActionCard('reset_to_basesetpoint').registerRunListener(async (args, state) => {
      await args.device.knxInterface.writeKNXGroupAddress(args.device.settings.ga_temperature_target, 0, 'DPT9.1')
        .catch((knxerror) => {
          this.log(knxerror);
